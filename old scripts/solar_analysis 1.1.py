...
# Current Electricity Price
future_daily_supply_charge = 1.221
future_per_kwh_charge = 0.2992
future_feed_in_tariff = 0.03
...
# print('Total saved: $' + str(analysed_solar_data_sorted.iloc[:, 9].sum()))
# print(analysed_solar_data_sorted)
# current_battery_savings_total = 0
# for index, row in analysed_solar_data_sorted.iterrows():
#     current_battery_savings = row[3] * (per_kwh_charge - feed_in_tariff)
#     current_battery_savings_total += current_battery_savings
#     print(row[0] + ' $' + str(round(current_battery_savings, 1)))
# print('Total Current battery savings: $'+ str(round(current_battery_savings_total, 1)))
# print('Total Annual Forecast Current battery savings: $'+ str(round(current_battery_savings_total*1.33, 1)))

# future_battery_savings_total = 0
# for index, row in analysed_solar_data_sorted.iterrows():
#     future_battery_savings = row[3] * (future_per_kwh_charge - future_feed_in_tariff)
#     future_battery_savings_total += future_battery_savings
#     # print(row[0] + ' $' + str(round(future_battery_savings, 1)))
# print('Total Future battery savings: $'+ str(round(future_battery_savings_total, 1)))
# print('Total Annual Forecast Future battery savings: $'+ str(round(future_battery_savings_total*1.3, 1)))

# future cost + additional energy usage - 200+200+100+100+70+0+0+200+100+100+200+200=1470 (do 0.8 for estimate error)
additional_energy_usage = 1470 * 0.8
additional_savings = additional_energy_usage * (future_per_kwh_charge - future_feed_in_tariff)
# print('Total Annual Forecast Future battery savings with additional power usage: $'+ str(round(future_battery_savings_total*1.3 + additional_savings, 1)))


# current data (oct,nov,dec prediction)
total_production = 800+850+950
total_load = 500+500+500

# future total combined savings 
total_load_saved = 500+500+500
total_export = 300+350+450
total_future_combined_saving = 0

for index, row in analysed_solar_data_sorted.iterrows():
    total_production += row[2]
    total_load += row[5]
    if '5' in row[0] or '6' in row[0] or '7' in row[0]:
        total_load_saved += row[2]
    else:
        total_load_saved += row[5]

print('Total Production: '+ str(round(total_production, 1)) + 'kWh')
print('Total Load: '+ str(round(total_load, 1)) + 'kWh')
print('Total Load Saved: '+ str(round(total_load_saved, 1)) + 'kWh')
print('Total Export: '+ str(round(total_export, 1)) + 'kWh')
print('Increased(x1.2 load) Total Load Saved: '+ str(round(total_load_saved*1.2, 1)) + 'kWh')
print('Increased(x1.2 load) Total Export: '+ str(round(total_production-total_load_saved*1.2, 1)) + 'kWh')

print('Current cost saving: $'+ str(round(total_load_saved*per_kwh_charge + (total_production-total_load_saved)*feed_in_tariff, 1)))
print('Future cost saving: $'+ str(round(total_load_saved*future_per_kwh_charge + (total_production-total_load_saved)*future_feed_in_tariff, 1)))
print('Increased(x1.2 load) Future cost saving: $'+ str(round(total_load_saved*1.2*future_per_kwh_charge + (total_production-total_load_saved*1.2)*future_feed_in_tariff, 1)))